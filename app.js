...
const checkoutWebhookHandler =
  require("./src/orders/controller/order.controller").checkoutWebhookHandler;
app.post(
  "/checkout-webhook",
...
const {
  ...
  globalErrorHandler,
} = require(`./common/middleware/error.handling.middleware`);
...
process.on(`unhandledRejection`, (error) => {
  console.error(
    `Unhandled Rejection error >> ${error.name} || ${error.message}`
  );
  server.close(() => {
